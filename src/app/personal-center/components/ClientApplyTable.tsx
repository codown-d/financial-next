  ...
  let columns: ProColumns<any>[] = [
    ...
    },
    {
      ...
      render: (text, record, _, action) => {
        let { product_type, product_id } = record;
        let f = getFile(product_type);
        return record?.attachment_list?.length ? (
          <DrawerForm
            title="附件"
            key={"info"}
            trigger={
              <Button type={"link"} className="!text-[#3d5af5]">
                查看详情
              </Button>
            }
            onFinish={async () => {
          ...
          <ModalForm
            title={<h2 className="text-center text-[24px] ">添加附件</h2>}
            trigger={
              <Button type={"link"} className="!text-[#3d5af5]">
                添加附件
              </Button>
            ...
            {product_type == FinanceDataTypeEmu.EquityFinancing ? (
              ...
              <>
                ...
                <ProFormDependency name={["file_type"]}>
                  {({ file_type }) => {
                    let arr = file_type == "gr" ? personalList : enterpriseList;
                    return (
                      <ProFormGroup>
                        {arr.map((item, index) => {
                          return (
